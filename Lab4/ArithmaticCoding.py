...
# ------------------------------
# Set Decimal Precision (high enough for large images)
# ------------------------------
getcontext().prec = 10000  # adjust precision if needed
...
def get_intervals(prob_table):
    ...
    Returns intervals as Decimal ranges.
    """
    intervals = {}
    low = Decimal(0)
...
def arithmetic_encode(data, prob_table):
    """
    Encode data using arithmetic coding with Decimal precision.
    """
...
def arithmetic_decode(encoded_value, length, prob_table):
    """
    Decode arithmetic-coded value with Decimal precision.
    """
    intervals = get_intervals(prob_table)
    result = []
    low, high = Decimal(0), Decimal(1)
...
def image_compression_arithmetic(img_path):
    ...
    # Step 7: Compression ratio (rough estimate)
    original_bits = rows * cols * 8
    compressed_bits = int(np.ceil(-np.log2(prob_table[min(prob_table, key=prob_table.get)]))) * len(data)
...
if __name__ == "__main__":
    image_compression_arithmetic("lena.png")  # replace with your image file
